...
import { Home } from '../';
import { useCallbackTrello } from '../../hooks';

function CallBackTrello() {
  const { navigate, success, loading } = useCallbackTrello();
  ...
  const showInfos = () => {
    return success ? (
      <div>
        <h1>Success !</h1>
        <span>
          You are connected to Trello's service. Go <a href='/home'>to the home page</a> to add new
      ...
      <div>
        <h1>Failure</h1>
        You weren't able to connect to Trello's service. Go{' '}
