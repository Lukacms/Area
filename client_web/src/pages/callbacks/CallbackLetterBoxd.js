...
import { Home } from '../';
import { useCallbackLetterBoxd } from '../../hooks';

function CallBackLetterBoxd() {
  const { navigate, success, loading } = useCallbackLetterBoxd();
  ...
  const showInfos = () => {
    return success ? (
      <div>
        <h1>Success !</h1>
        <span>
          You are connected to LetterBoxd's service. Go <a href='/home'>to the home page</a> to add
      ...
      <div>
        <h1>Failure</h1>
        You weren't able to connect to LetterBoxd's service. Go{' '}
...
export default CallBackLetterBoxd;
