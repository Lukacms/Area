...
import { Home } from '../';
import { useCallbackOutlook } from '../../hooks';

function CallBackOutlook() {
  const { navigate, success, loading } = useCallbackOutlook();
  ...
  const showInfos = () => {
    return success ? (
      <div>
        <h1>Success !</h1>
        <span>
          You are connected to Outlook's service. Go <a href='/home'>to the home page</a> to add new
      ...
      <div>
        <h1>Failure</h1>
        You weren't able to connect to Outlook's service. Go{' '}
...
export default CallBackOutlook;
