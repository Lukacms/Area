...
public class Tests
{
    ...
    public void TestValidLogin()
    {
        ...
        var retUnautor = new UnauthorizedObjectResult(null);
        var retBad = new BadRequestObjectResult(new JsonObject());
        var retnotfound = new NotFoundObjectResult(null);
        if (test.GetType() == retUnautor.GetType())
            Assert.Fail();
        if (test.GetType() == retBad.GetType())
            Assert.Fail();
